...
def setup_train_args():
    ...
    设置训练参数
    """
    # argparse是一个提供更轻松的命令行指令编写的库，这里主要用来做变量管理
...
def create_model(args, vocab_size):
    ...
    if args.pretrained_model:  # 如果指定了预训练的GPT2模型
        ...
        # 4.5.1版本transformers.models.gpt2.modeling_gpt2，将本地config文件通过GPT2Config方法添加到GPT2模型的总配置中
        model_config = transformers.models.gpt2.modeling_gpt2.GPT2Config.from_json_file(args.model_config)
        # 通过上条语句的配置初始化GPT2
        model = GPT2LMHeadModel(config=model_config)
    # 将GPT2的词典大小调整为本地词典大小13317
...
def preprocess_raw_data(args, tokenizer, n_ctx):
    ...
    with open(args.train_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):  # tqdm进度条插件
            if "\r\n" in data:
            ...
            for utterance in utterances:
                # 对于每个utterance，将其中内容逐字转化成tokenize_id的形式
...
def main():
    ...
    # 通过args创建的日志，不仅将日志信息写入log文件还能在控制台输出
    global logger
    ...
    # 新实验开始
    logger.info('new experiment start:{}'.format('  ******************************************************************\n'))
    # 打印1：是否使用cuda
